...
update_actions (NemoView *view)
{
    ...
    gchar *escaped_path = g_strdup_printf("Actions");
    ...
    action_list = nemo_action_manager_list_actions (NEMO_WINDOW (view->details->window)->action_manager);

    for (node = action_list; node != NULL; node = node->next) {
        action = node->data;
        add_action_to_action_menus (view, action, menu_path, popup_path, popup_bg_path);
    }
